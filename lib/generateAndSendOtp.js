...
const generateAndSendOtp = async (user) => {
  // If email isn't configured in production, don't crash auth flows.
  // The API will still generate/store an OTP; sending can be retried after config is fixed.
...
<div style="font-family: Arial, sans-serif; background-color: #f4f4f5; padding: 40px; text-align: center;" dir="ltr">
  <div style="max-width: 500px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; padding: 40px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
   ...
   if (!mailUser || !mailPass) {
     console.warn("OTP email skipped: EMAIL/PASS not set");
   ...
   try {
     ...
     await Promise.race([
       ...
       new Promise((_, reject) =>
         setTimeout(() => reject(new Error("OTP send timeout")), 6000)
       ),
     ]);
     return { ok: true, emailSent: true };
   } catch (err) {
     console.error("generateAndSendOtp sendMail error:", err);
     return { ok: true, emailSent: false };
   }
}
